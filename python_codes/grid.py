...
class Tac(object):
    ...
    def __init__(self, HEIGHT=HEIGHT, WIDTH=WIDTH, BGFILL=BGcolor):
    ...
    def draw_player_fig(self, pos):
        ...
        if self.player == 2:
            ...
            py.draw.circle(self.screen, self.playerCol, (xpos, ypos), RADIUS, lineWidth)
            ...
            py.draw.line(self.screen, self.playerCol, (x_start, y_start), (x_end, y_end), lineWidth * 2)
            py.draw.line(self.screen, self.playerCol, (x_start, y_end), (x_end, y_start), lineWidth * 2)

    def draw_vline(self, pos):
        py.draw.line(self.screen, self.playerCol, (SQ_WIDTH * pos[1] + SQ_WIDTH / 2, OFFSET),
                     (SQ_WIDTH * pos[1] + SQ_WIDTH / 2, HEIGHT - OFFSET), lineWidth)

    def draw_hline(self, pos):
        py.draw.line(self.screen, self.playerCol, (OFFSET, SQ_HEIGHT * pos[0] + SQ_HEIGHT / 2),
                     (WIDTH - OFFSET, SQ_HEIGHT * pos[0] + SQ_HEIGHT / 2), lineWidth)

    def draw_asc_line(self):
        py.draw.line(self.screen, self.playerCol, (OFFSET, OFFSET), (WIDTH - OFFSET, HEIGHT - OFFSET), lineWidth*2)

    def draw_dec_line(self):
        py.draw.line(self.screen, self.playerCol, (OFFSET, HEIGHT - OFFSET), (WIDTH - OFFSET, OFFSET), lineWidth*2)
